...
class LemmyCommentsHelper {
  ...
  public constructor(comments: CommentView[]) {
    this.unparsedComments = comments.sort((a, b) =>
  ...
  private addReplies = (lemmyComment: ILemmyComment): void => {
    const comments = this.unparsedComments.filter((x) =>
      x.comment.path.startsWith(lemmyComment.top.comment.path)
    );
    ...
    for (const comment of comments) {
      ...
      if (
        pathArr[pathArr.length - 2] !== lemmyComment.top.comment.id.toString()
      )
        continue;
  ...
  public static findAndAdd = (
    chain: ILemmyComment[],
    newComment: ILemmyComment,
    depth = 1
  ): ILemmyComment[] => {
    ...
    for (const comment of chain) {
      if (comment.top.comment.path.includes(pathArr[depth])) {
        if (comment.top.comment.id.toString() === pathArr[pathArr.length - 2]) {
          ...
          comment.replies = this.findAndAdd(
            comment.replies,
            newComment,
            depth + 1
          );
